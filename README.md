...
2. 챗봇 응답대기 시간을 줄일 수 있는 방안을 모색
  ...
  - 게임 시작 시 각 유저는 자신의 능력치 및 설정을 정한다.
  - 각 유저의 수에 따라 턴이 생기고, 각자의 턴에만 input창이 활성화 되도록 만드려고 함.
  - 게임 시작 후, 활성화되는 채팅창에서 처음에는 세계관 설명 -> 각 유저의 턴마다 ai를 활용해서 trpg를 이어가도록 학습시키기.

  ### 12/5
  구현을 잠시 멈추고 현재까지의 코드를 이해하는 시간을 가지려 함.

  - socket.io의 구조에 대해서 이해해보며, 실습 해보기.
  - genai 활용방법에 대해서 공식문서와 함께 ai를 활용하지 않고, 작성해보기.
  - 새로운 폴더를 만들어서 진행.

  - socket.io를 ai가 짜준 코드를 복붙하지 말고, 이해하려 함.
  - socket.io(http서버(express모듈)) 형태로 사용됨.
  - io.on('connection', () => {})을 통해서 소켓 통신이 되었을 때, 사용자 접속 표시 
  - io.on('disconnect', () => {})을 소켓 통신이 끊겼을 때, 접속 해제 표시